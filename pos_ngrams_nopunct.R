...
dst_root <- file.path(base, "train_15k_ngrams_nopunct")
...
process_chunk <- function(tsv_path) {
  ...
  # Strip PUNCT tokens before building n-grams
  pos_seq <- df$upos[!is.na(df$upos) & df$upos != "PUNCT"]
...
author_counts <- all_chunks %>%
  ...
  mutate(total_tokens = total_tokens_author, rel_freq = count / total_tokens)
